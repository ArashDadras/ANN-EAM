...
transformed parameters {
    ...
    for (n in 1:N) {
        drift_word_t[n] = (k_1[n] + b[n] * frequency[n]) / (1 + exp(-alpha * (p[n][1]-0.5)));
        drift_nonword_t[n] = k_2[n] / (1 + exp(-alpha * (p[n][2]-0.5)));
    }
}
...
model {
    ...
    RT ~ race(ndt, threshold_word, threshold_nonword, drift_word_t, drift_nonword_t);
}
