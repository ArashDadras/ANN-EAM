...
behavioural_df = pd.read_csv(behavioural_data_root + 'LDT_data.csv',
                             header=None,
                             names=['accuracy', 'rt', 'string', 'response', 'participant',
                                    'minRT', 'participant_id'])
...
# For models wtih non-decision time modulation
m_priors = [0, 0.5, 0, 1]
# For models without drift mapping functions (non ANN-EAM models)
...
b_priors = [0, 1, 1, 1]
# For models with sigmoid drift mapping functions (ANN-EAM models)
k_priors = [2, 1, 1, 1]
