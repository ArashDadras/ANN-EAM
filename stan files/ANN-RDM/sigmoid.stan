// ANN-RDM with sigmoid mapping function for drifts without asymptote modulation,
...
transformed parameters {
    ...
    for (n in 1:N) {
        drift_word_t[n] = k_1_sbj[participant[n]] / (1 + exp(-alpha_sbj[participant[n]] * (p[n][1]-0.5)));
        drift_nonword_t[n] = k_2_sbj[participant[n]] / (1 + exp(-alpha_sbj[participant[n]] * (p[n][2]-0.5)));
